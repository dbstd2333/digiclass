...
const _require$1 = node_module.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('chunks/lib-ddb42891.js', document.baseURI).href)));
    ...
    return [
        {
            ...
            config(config) {
                ...
                defaultConfig.build.rollupOptions.output['assetFileNames'] = path__default["default"].posix.join(build.assetsDir || defaultConfig.build.assetsDir, '[name]-[hash].[ext]');
...
function toRelativePath(filename, importer) {
    const relPath = path__default["default"].posix.relative(path__default["default"].dirname(importer), filename);
...
function assetPlugin() {
    ...
    return {
        ...
        configResolved(config) {
            ...
            outDir = vite.normalizePath(config.build.outDir);
        },
        resolveId(id) {
            if (id === wasmHelperId) {
                return id;
        ...
        async load(id) {
            ...
            if (publicDir && file.startsWith(publicDir)) {
                ...
                if (cached) {
                    ...
                    const source = await fs__default$1["default"].readFile(file);
        ...
        renderChunk(code, chunk) {
            ...
            if (code.match(nodeAssetRE)) {
                while ((match = nodeAssetRE.exec(code))) {
                    s || (s = new MagicString__default["default"](code));
            ...
            if (code.match(nodePublicAssetRE)) {
                while ((match = nodePublicAssetRE.exec(code))) {
                    ...
                    const outputFilepath = toRelativePath(filename, vite.normalizePath(path__default["default"].join(outDir, chunk.fileName)));
                    const replacement = JSON.stringify(outputFilepath);
                    s.overwrite(match.index, match.index + full.length, replacement, {
                        contentOnly: true
                    });
...
const _require = node_module.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('chunks/lib-ddb42891.js', document.baseURI).href)));
