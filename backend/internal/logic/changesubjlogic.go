...
func (l *ChangeSubjLogic) ChangeSubj(req *types.ChangeSubjReq) (resp *types.ChangeSubjResp, err error) {
	...
	l.svcCtx.Redis.Del(req.ClassCode + time.Weekday(week).String())
	//var sbj svc.Subject
	sbjnames := svc.Sbjjson{
	...
	sbjnamebyte, err := json.Marshal(sbjnames)
	teacherbyte, err := json.Marshal(teachers)
	srcbyte, err := json.Marshal(srcs)
	subject := &Subject{
		ClassCode: req.ClassCode,
		Weekly:    week,
		Sbjname:   string(sbjnamebyte),
